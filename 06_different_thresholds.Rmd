...
for (f in filters) {
  ...
  df <- df_subset %>%
    ...
    select(sample, genome, value) %>%   # 👈 replace genome if needed
  ...
  beta_list[[f]] <- hillpair(mat, q = 1)
}
save(beta_list, file="resources/amplicon/distance_thresholds.Rdata")

```

```{r}
load("resources/amplicon/distance_thresholds.Rdata")
...
# Inspect results, ordered by best correlation
mantel_results %>%
...
pca_df <- as.data.frame(pca_result$x) %>%
  ...
  separate(filter_type, into = c("filter", "type"), sep = "_")
```
