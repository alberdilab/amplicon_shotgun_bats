...
#ggsave("Figures_ms/beta_richness_metagenomics.pdf", plot = p, width = 8, height = 4)
...
#ggsave("Figures_ms/beta_neutral_metagenomics.pdf", plot = p, width = 8, height = 4)
```
...
adonis2(beta_q0n$S ~ Species*method, data = all_samples %>% filter(sample %in% labels(beta_q0n$S)) %>% arrange(match(sample, labels(beta_q0n$S))), permutations = 999) %>%
  broom::tidy() %>%
  tt()
...
adonis2(beta_q0n_phylum$S ~ Species*method, data = all_samples %>% filter(sample %in% labels(beta_q0n_phylum$S)) %>% arrange(match(sample, labels(beta_q0n_phylum$S))), permutations = 999) %>%
  broom::tidy() %>%
  tt()
...
sample_metadata_row <- all_samples %>%
    filter(sample %in% labels(beta_q1n_phylum$S)) %>%
    arrange(match(sample, labels(beta_q1n_phylum$S))) %>% 
  column_to_rownames(., "sample") 
            
pairwise.adonis2(beta_q1n_phylum$S ~ Species, data=sample_metadata_row, perm = 999)
```
