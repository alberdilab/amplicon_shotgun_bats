...
### Aggregate counts at family level in both datasets
```{r}
load("resources/amplicon/data_standard_filt.Rdata")
...
genome_metadata_ampli <- genome_metadata %>%
  mutate(
    ...
    order  = if_else(is.na(order), paste0(class, "_Unclassified"), order),
    family = if_else(is.na(family), paste0(order, "_Unclassified"), family),
    genus  = if_else(is.na(genus), paste0(family, "_Unclassified"), genus)
  )
...
genome_metadata <- genome_metadata %>%
  mutate(
    class = if_else(is.na(class) | str_trim(class) == "", paste0(phylum, "_Unclassified"), class),
    order = if_else(is.na(order) | str_trim(order) == "", paste0(class, "_Unclassified"), order),
    family = if_else(is.na(family) | str_trim(family) == "", paste0(order, "_Unclassified"), family),
    genus = if_else(is.na(genus) | str_trim(genus) == "", paste0(family, "_Unclassified"), genus)
  )
...
```{r beta_div_together, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=16, fig.fullwidth=TRUE}
...
```
```{r permanova_rich_both_fig2, comment="", message=FALSE, warning=FALSE}
...
***Neutral diversity***
```{r beta_div1_both_fig3, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=16, fig.fullwidth=TRUE}
beta_q1n$S %>%
  vegan::metaMDS(.,
                 trymax = 500,
                 k = 2,
...
```
```{r beta_div1_both_appr_fig4, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=16, fig.fullwidth=TRUE}
beta_q1n$S %>%
  vegan::metaMDS(.,
                 trymax = 500,
                 k = 2,
...
```{r permanova_neutral_tog, comment="", message=FALSE, warning=FALSE}
betadisper(
  beta_q1n$S,
  all_samples %>%
...
```
```{r beta_div1_both_fig5, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=16, fig.fullwidth=TRUE}
beta_q1n_ampli$S %>%
  vegan::metaMDS(.,
                 trymax = 500,
                 k = 2,
...
***Shotgun***
```{r pairwise_neutral_to, comment="", message=FALSE, warning=FALSE}
...
```{r beta_div1_both_fig7, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=16, fig.fullwidth=TRUE}
beta_q1n_shot$S %>%
  vegan::metaMDS(.,
                 trymax = 500,
                 k = 2,
