...
# anova(Model_richness)
# summary(Model_richness)
Modelq0GLMMNB <- MASS::glm.nb(richness ~ Species*method, data = alpha_div_family_data,trace=TRUE)
summary(Modelq0GLMMNB)
```
```{r}
Model_richness <- glmer.nb(richness ~ Species*method+(1|sample), data = alpha_div_family_data)
summary(Model_richness)
anova(Model_richness)
```
...
```
```{r}
Model_richness_random <- glmer.nb(richness ~ Species*method+(1|sample), data = alpha_div_phylum_data)
summary(Model_richness_random)
anova(Model_richness_random)
```
```{r}
AIC(Model_richness_phylum, Model_richness_random)
#check if the model actually converged
summary(Model_richness_random)$optinfo$conv$lme4$messages #NULL: no serious issue
#Check for singular fit
isSingular(Model_richness_random, tol = 1e-4) #FALSE: random structure is fine
...
overdisp_fun(Model_richness_random) #~1: good (1.2 → overdispersion, <0.8: possible underdispersion) 
```
