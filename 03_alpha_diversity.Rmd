...
```{r meta_mix, comment="", message=FALSE, warning=FALSE}
...
***Neutral***
```{r neutral_MM_species, comment="", message=FALSE, warning=FALSE}
...
```{r meta_ampli_mm, comment="", message=FALSE, warning=FALSE}
alpha_div_meta <- alpha_div %>%
  left_join(sample_metadata, by = join_by(sample == sample))
```

***Richness***
```{r rich_MM_ampli, comment="", message=FALSE, warning=FALSE}
Model_richness <- lm(formula = richness ~ Species, data = alpha_div_meta)
anova(Model_richness)
```

***Neutral***
```{r neutral_MM_ampli, comment="", message=FALSE, warning=FALSE}
Model_neutral <- lm(formula = neutral ~ Species, data = alpha_div_meta)
anova(Model_neutral)
```
