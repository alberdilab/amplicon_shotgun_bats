...
```{r}
fraction <- read_csv("resources/metagenomics/microbial_fraction_estimation.csv") %>% 
...
fraction %>% 
  summarise(
    Total_mean = mean(read_fraction, na.rm = T),
    Total_sd = sd(read_fraction, na.rm = T),
    Eb_mean = mean(read_fraction[Species == "Eb"], na.rm = T),
    Eb_sd = sd(read_fraction[Species == "Eb"], na.rm = T),
    Ha_mean = mean(read_fraction[Species == "Ha"], na.rm = T),
    Ha_sd = sd(read_fraction[Species == "Ha"], na.rm = T),
    Pk_mean = mean(read_fraction[Species == "Pk"], na.rm = T),
    Pk_sd = sd(read_fraction[Species == "Pk"], na.rm = T)
  ) %>%
  mutate(
...
```
```{r}
  kruskal.test(read_fraction ~ Species, data = fraction)
```

Correlation between diversity and estimate microbial fraction
```{r}
cor.test(fraction$read_fraction, fraction$richness, method = "spearman")
cor.test(fraction$read_fraction, fraction$neutral, method = "spearman")

# Histogram
hist(fraction$read_fraction, main = "Read Fraction", xlab = "read_fraction")
hist(fraction$richness, main = "Richness", xlab = "richness")

# QQ plots
qqnorm(fraction$read_fraction); qqline(fraction$read_fraction)
qqnorm(fraction$richness); qqline(fraction$richness)

shapiro.test(fraction$read_fraction)
shapiro.test(fraction$richness)
```

```{r fig_fraction_mergedmeta, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=16, fig.fullwidth=TRUE}
p1 <- ggplot(fraction, aes(x = read_fraction, y = richness)) +
  geom_point(size = 3, color = "#2E86AB") +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  theme_classic() +
  labs(x = "Read Fraction (%)", y = "Richness")

p2 <- ggplot(fraction, aes(x = read_fraction, y = neutral)) +
...
#ggsave("Figures_ms/fraction_diversity_metagenomics.pdf", plot = p_both_meta, width = 10, height = 6)
...
Model_richness <- lm(formula = richness ~ Species, data = alpha_div_f_metag_data)
anova(Model_richness)
emmeans(Model_richness, pairwise ~ Species)
```
...
Model_richness <- lm(formula = richness ~ Species, data = alpha_div_phylum_metag_data)
anova(Model_richness)
emmeans(Model_richness, pairwise ~ Species)
```
...
```{r save_alpha, comment="", message=FALSE,echo=FALSE,warning=FALSE}
save(merged_table, 
     merged_table_phylum,
     all_samples,
...
Family level
```{r fig_family_comp, comment="", message=FALSE, warning=FALSE, echo=FALSE, fig.height=6, fig.width=16, fig.fullwidth=TRUE}
...
Model_richness <- lm(formula = richness ~ Species*method, data = alpha_div_family_data)
anova(Model_richness)
...
Model_neutral <- lm(formula = neutral ~ Species*method, data = alpha_div_family_data)
anova(Model_neutral)
...
```{r gif_phylum_amp, comment="", message=FALSE, warning=FALSE, echo=FALSE, fig.height=6, fig.width=16, fig.fullwidth=TRUE}
...
Model_richness_phylum <- lm(formula = richness ~ Species*method, data = alpha_div_phylum_data)
...
Model_neutral_phylum <- lm(formula = neutral ~ Species*method, data = alpha_div_phylum_data)
