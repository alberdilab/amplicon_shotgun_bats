...
#ggsave("Figures_ms/taxonomy_metagenomics.pdf", plot = p, width = 10, height = 6)
...
microbial_fraction %>%
  ...
  inner_join(sample_metadata,by="sample") %>% 
	ggplot(., aes(x = sample, y = value, fill=fraction)) +
      ...
      facet_grid(cols=vars(Species), space="free", scales="free") +
...
#ggsave("Figures_ms/taxonomy_metabarcoding_standard.pdf", plot = p, width = 10, height = 6)
...
summary_table_ampli %>% 
  select(-Phylum,-Family, -Genus) %>% 
  rowwise() %>% 
  mutate(ASV_perc=round(ASVs*100/3834, 2))%>% 
...
### Differences in top phyla between methods
...
# 
# phylo_genome_metagenome <- genome_metadata %>%
#     column_to_rownames(var="genome")%>%
#   as.matrix()%>%
#   tax_table()
# 
# physeq_genome_filt <- phyloseq(phylo_genome,phylo_genome_metagenome)
# physeq_genome_filt_clr <- microbiome::transform(physeq_genome_filt, "clr")
# 
# topFMAG_clr <- data.frame(physeq_genome_filt_clr@otu_table) %>%
#   rownames_to_column(., "genome") %>%
#   left_join(data.frame(physeq_genome_filt_clr@tax_table) %>%
#   rownames_to_column(., "genome") %>%
#       select(1, 3), by="genome") %>%
#   select(-genome) %>% 
#   group_by(phylum) %>% 
#   summarize(across(where(is.numeric), sum), .groups = "drop") %>% 
#   mutate(total_mag = rowSums(across(where(is.numeric)))) %>% 
...
CLR
```{r}
# phylo_genome_amplicon <- genome_counts_filt_amplicon %>% 
#   column_to_rownames("genome") %>% 
#   otu_table(., taxa_are_rows = TRUE)
# 
# phylo_tax_amplicon <- genome_metadata_amplicon %>%
#     column_to_rownames(var="genome")%>%
#   as.matrix()%>%
#   tax_table()
# 
# physeq_amplicon_filt <- phyloseq(phylo_genome_amplicon,phylo_tax_amplicon)
# physeq_amplicon_filt_clr <- microbiome::transform(physeq_amplicon_filt, "clr")
# 
# topFASV_clr <- data.frame(physeq_amplicon_filt_clr@otu_table) %>%
#   rownames_to_column(., "genome") %>%
#   left_join(data.frame(physeq_amplicon_filt_clr@tax_table) %>%
#   rownames_to_column(., "genome") %>%
#       select(1, 3), by="genome") %>%
#   select(-genome) %>% 
#   group_by(phylum) %>% 
...
#   slice_head(n = 10)
# 
# table_clr <- full_join(topFMAG_clr,topFASV_clr, by="phylum") %>% 
#     mutate(ASV = replace_na(total_asv, 0)) %>% 
#     mutate(MAG = replace_na(total_mag, 0)) %>% 
#   select(-total_asv, -total_mag) %>% 
#   pivot_longer(cols = c(MAG, ASV),
...
```{r figure_scat_family, fig.height=8, fig.width=12, fig.fullwidth=TRUE}
...
load("resources/metagenomics/data_filtered.Rdata")
```
```{r histogram_abundace_together_mags, fig.height=6, fig.width=18, fig.fullwidth=TRUE}
genome_counts_filt %>%
  mutate(across(-genome, ~ . / sum(.))) %>%          # normalize each genome
  column_to_rownames("genome") %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  select(-sample) %>%                      # remove grouping info
  summarise(across(everything(), mean, na.rm = TRUE)) %>%  # mean across all samples
  pivot_longer(
    cols = everything(),
    names_to = "Genome",
    values_to = "MeanValue"
  ...
  ggplot(., aes(x = reorder(Genome, -MeanValue), y = MeanValue)) +
  geom_col(fill = "steelblue") +
  theme_minimal() +
  labs(
    x = "MAGs",
    y = "Mean Value",
    title = "Overall Mean Genome Abundance (All Samples Combined)"
  ) +
...
```
```{r histogram_abundace_mag, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
genome_counts_filt %>%
  mutate(across(-genome, ~ . / sum(.))) %>%
  column_to_rownames(., "genome") %>% 
  t() %>% 
  as.data.frame() %>% 
  rownames_to_column(., "sample") %>% 
  left_join(sample_metadata[c(1, 2)], by="sample") %>% 
  select(-sample) %>% 
  group_by(Species) %>% 
  summarise(across(where(is.numeric), mean, na.rm = TRUE)) %>% 
  pivot_longer(
    cols = -Species,                # all columns except Species
    names_to = "Genome",            # column names become a variable
    values_to = "MeanValue"         # cell values become this variable
  ) %>%
  group_by(Genome) %>%
  mutate(overall_mean = mean(MeanValue, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(Genome = fct_reorder(Genome, overall_mean, .desc = TRUE)) %>% 
  ggplot(., aes(x = Genome, y = MeanValue, fill = Species)) +
  ...
  labs(
    ...
    y = "Mean Value",
    fill = "Species",
    title = "Mean Genome Abundance per Species"
  ) +
  theme(axis.text.x = element_blank(),
        ...
        legend.title = element_text(size = 18))
```
```{r together_prev_mag, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
presence_threshold <- 0  # define what counts as "present"
...
genome_counts_filt %>%
  ...
  left_join(sample_metadata[c("sample", "Species")], by = "sample") %>%
  pivot_longer(
    cols = -c(sample, Species),
    names_to = "Genome",
    values_to = "RelAbundance"
  ) %>%
  group_by(Genome) %>%
  summarise(
    MeanAbundance = mean(RelAbundance, na.rm = TRUE),
    Prevalence = sum(RelAbundance > presence_threshold, na.rm = TRUE),
    FractionPrevalence = Prevalence / n(),  # optional normalized prevalence
    .groups = "drop"
  ) %>% 
  ggplot(., aes(x = fct_reorder(Genome, -MeanAbundance), y = MeanAbundance, fill = Prevalence)) +
  geom_col() +
  ...
  labs(
    ...
    fill = "Prevalence",
    title = "Mean MAG relative abundance colored by prevalence"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
```{r log_transfe, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
presence_threshold <- 0  # define what counts as "present"

genome_summary_abs <- genome_counts_filt %>%
  column_to_rownames("genome") %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  ...
  group_by(Genome) %>%
  summarise(
    MeanAbsCount = mean(AbsCount, na.rm = TRUE),
    Prevalence = sum(AbsCount > presence_threshold, na.rm = TRUE),
    FractionPrevalence = Prevalence / n(),
    .groups = "drop"
  ) %>%
  mutate(LogMeanAbsCount = log10(MeanAbsCount + 1)) 
ggplot(genome_summary_abs, aes(
  x = fct_reorder(Genome, MeanAbsCount, .desc = TRUE),
  y = LogMeanAbsCount,
  fill = FractionPrevalence
)) +
  geom_col() +
  ...
  labs(
    ...
    y = "log10",
    fill = "Prevalence",
    title = "Mean MAG Abundance (Absolute Counts, log-transformed) Colored by Prevalence"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
genome_prevalence_counts <- genome_counts_filt %>%
  mutate(across(-genome, ~ . / sum(.))) %>%              # normalize within genome if needed
  column_to_rownames("genome") %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  ...
  group_by(Genome) %>%
  summarise(
    Prevalence = sum(RelAbundance > presence_threshold, na.rm = TRUE),  # count of samples
    TotalSamples = n(),                                                  # total samples in that species
    Fraction = Prevalence / TotalSamples,                                # optional: proportion
    .groups = "drop"
  )
```
```{r}
ggplot(genome_prevalence_counts, aes(x = reorder(Genome, -Prevalence), y = Prevalence)) +
  geom_col(position = "dodge") +
  theme_minimal() +
  labs(
    x = "MAGs",
    y = "Prevalence",
    title = "MAGs Prevalence Within Each Species"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
        ...
        legend.title = element_text(size = 12))
```
```{r histogram_abundace, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
genome_counts_filt %>%
  mutate(across(-genome, ~ . / sum(.))) %>%
  column_to_rownames(., "genome") %>% 
  t() %>% 
...
```{r}
presence_threshold <- 0
genome_prevalence_counts_species <- genome_counts_filt %>%
  mutate(across(-genome, ~ . / sum(.))) %>%              # normalize within genome if needed
  column_to_rownames("genome") %>%
  t() %>%
  as.data.frame() %>%
...
```
```{r}
genome_prevalence_counts_species %>% 
  filter(Species=="Eb") %>% 
    filter(Prevalence!=0) %>% 
ggplot(., aes(x = reorder(Genome, -Prevalence), y = Prevalence)) +
  geom_col(position = "dodge") +
  theme_minimal() +
  labs(
    x = "MAGs",
    y = "Prevalence",
    title = "MAGs prevalence within Cnephaeus"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
ggplot(., aes(x = reorder(Genome, -Prevalence), y = Prevalence)) +
  ...
  labs(
    x = "MAGs",
    y = "Prevalence",
    title = "MAGs prevalence within Hypsugo"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
ggplot(., aes(x = reorder(Genome, -Prevalence), y = Prevalence)) +
  ...
  labs(
    x = "MAGs",
    y = "Prevalence",
    title = "MAGs prevalence within Pipistrellus"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
```{r histogram_abundace_together, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
genome_counts_filt %>%
  mutate(across(-genome, ~ . / sum(.))) %>%          # normalize each genome
  column_to_rownames("genome") %>%
  t() %>%
...
```
```{r together_prev, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
presence_threshold <- 0  # define what counts as "present"
...
genome_counts_filt %>%
  ...
  labs(
    ...
    y = "Mean relative abundance",
    fill = "Prevalence",
    title = "Mean relative abundance of top 200 ASVs colored by prevalence"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
```{r log_transf, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
presence_threshold <- 0  # define what counts as "present"
...
ggplot(genome_summary_abs, aes(
  ...
  labs(
    ...
    y = "log10",
    fill = "Prevalence",
    title = "Mean ASV Abundance (Absolute Counts, log-transformed) Colored by Prevalence"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
        ...
        legend.title = element_text(size = 12))
```
```{r together, fig.height=6, fig.width=12, fig.fullwidth=TRUE}
presence_threshold <- 0  # define what counts as "present"
...
ggplot(., aes(x = reorder(Genome, -Prevalence), y = Prevalence)) +
  ...
  labs(
    x = "ASVs",
    y = "Prevalence",
    title = "ASVs prevalence within Cnephaeus"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
ggplot(., aes(x = reorder(Genome, -Prevalence), y = Prevalence)) +
  ...
  labs(
    x = "ASVs",
    y = "Prevalence",
    title = "ASVs prevalence within Hypsugo"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
...
ggplot(., aes(x = reorder(Genome, -Prevalence), y = Prevalence)) +
  ...
  labs(
    x = "ASVs",
    y = "Prevalence",
    title = "ASVs prevalence within Pipistrellus"
  ) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"),
